...
export const GENERATOR_TEMPLATES: Record<string, GeneratorTemplate> = {
  ...
  "it-ticket-notes": {
    id: "it-ticket-notes",
    title: "IT ticket notes",
    fields: [
      { id: "ticket", label: "Ticket ID" },
      { id: "user", label: "User / asset" },
      { id: "symptom", label: "Symptom", multiline: true },
      { id: "steps", label: "Steps tried", multiline: true },
    ],
    build: (v) =>
      [
        `Ticket: ${val(v, "ticket", "—")}`,
        `User / asset: ${val(v, "user", "—")}`,
        "",
        "## Symptom",
        val(v, "symptom", "—"),
        "",
  ...
  "asset-handover": {
    ...
    fields: [
      { id: "asset", label: "Asset tag / serial" },
      { id: "from", label: "Handing over from" },
      { id: "to", label: "Receiving party" },
      { id: "condition", label: "Condition notes", multiline: true },
    ],
    build: (v) =>
      [
        "# Asset handover",
        ...
        "",
        "Signatures:",
        "From ______________   Date ______",
        "To   ______________   Date ______",
      ].join("\n"),
  },
  "software-install-request": {
    id: "software-install-request",
    title: "Software install request",
    fields: [
      { id: "app", label: "Application" },
      { id: "version", label: "Version / edition" },
      { id: "business", label: "Business justification", multiline: true },
      { id: "users", label: "Impacted users / devices" },
    ],
    build: (v) =>
      [
        "# Software install request",
        "",
        `App: ${val(v, "app", "—")} (${val(v, "version", "latest stable")})`,
        `Users / devices: ${val(v, "users", "—")}`,
        "",
        "## Justification",
        val(v, "business", "—"),
        "",
        "## Approvals",
        "Manager: ______   IT: ______   Security: ______ (if required)",
      ].join("\n"),
  },
  "distribution-list-request": {
    id: "distribution-list-request",
    title: "Distribution list / group request",
    fields: [
      { id: "name", label: "Requested name" },
      { id: "owners", label: "Owners (min 2)" },
    ...
    build: (v) =>
      [
        ...
        `Owners: ${val(v, "owners", "—")}`,
        "",
        "## Purpose / membership rules",
        val(v, "purpose", "—"),
        "",
        "## Delivery settings",
        "External senders allowed: yes / no",
  ...
  "incident-report": {
    ...
    fields: [
      { id: "title", label: "Incident title" },
      { id: "severity", label: "Severity (SEV)" },
      { id: "summary", label: "Executive summary", multiline: true },
      { id: "impact", label: "Customer / business impact", multiline: true },
    ],
    build: (v) =>
      [
        `# Incident — ${val(v, "title", "Untitled")}`,
        `Severity: ${val(v, "severity", "TBD")}`,
  ...
  "campaign-brief": {
    ...
    fields: [
      ...
      { id: "objective", label: "Objective + KPI", multiline: true },
      { id: "audience", label: "Audience / segments" },
      { id: "timeline", label: "Timeline / flight dates" },
    ],
    build: (v) =>
      [
        `# Campaign brief — ${val(v, "name", "Campaign")}`,
        "",
        "## Objective & KPI",
        val(v, "objective", "—"),
        "",
  ...
  "blog-outline": {
    ...
    fields: [
      ...
      { id: "audience", label: "Audience" },
      { id: "angle", label: "Angle / promise", multiline: true },
    ],
    build: (v) =>
      [
        `# Outline: ${val(v, "topic", "Topic")}`,
        `Audience: ${val(v, "audience", "Readers")}`,
        "",
        "## Hook",
        val(v, "angle", "—"),
        "",
  ...
  "daily-lead-check-report": {
    id: "daily-lead-check-report",
    title: "Daily lead-check report",
    fields: [
      { id: "date", label: "Date" },
      { id: "volume", label: "Lead volume / anomalies", multiline: true },
      { id: "sla", label: "SLA / speed-to-lead notes", multiline: true },
      { id: "actions", label: "Actions for tomorrow", multiline: true },
    ],
    build: (v) =>
      [
        `# Daily lead check — ${val(v, "date", new Date().toLocaleDateString())}`,
        "",
        "## Volume & anomalies",
        val(v, "volume", "—"),
        "",
