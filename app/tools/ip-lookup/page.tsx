...
import { scoreIPReputation } from "@/lib/risk-engine";
...
export default function IPLookupPage() {
  ...
  async function handleSubmit(ip: string) {
    ...
    setData(null);
    try {
      const { data, mock } = await import("@/lib/lookup-client").then((m) => m.lookupIP(ip));
      setData(data);
      setIsMock(mock);
    } catch (err: unknown) {
    ...
    >
      ...
      {!loading && !error && data && (
        <div className="space-y-4">
          <IPReputationCard data={data} />
          <RiskScorePanel risk={scoreIPReputation(data)} />
        </div>
      )}
      {!loading && !error && !data && (
        <ToolEmptyState
